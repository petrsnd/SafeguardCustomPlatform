...
> The reserved names below are the ones SPP populates automatically. Use these exact names in your parameter declarations.

| Parameter | Meaning in the workflow |
| --- | --- |
| `NewSshKey` | New public key to deploy during `ChangeSshKey`. |
| `NewSshKeyType` | Key algorithm for the new key, such as RSA or ED25519. |
| `NewSshKeyComment` | Comment to append when writing the new public key line. |
| `NewSshPrivateKey` | New private key generated by SPP (Secret type). |
| `OldSshKey` | Existing public key to verify or remove. |
| `OldPrivateSshKey` | Previous private key, if your workflow needs validation or rollback logic (Secret type). |
