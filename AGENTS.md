...
- **Do not ask "are you logged in yet?".** `Connect-Safeguard -DeviceCode` (or `-Browser`) blocks until login completes (or fails); await the cmdlet's own success/failure signal rather than polling the operator. Persist the same PowerShell session across iterations so re-login is rare.
